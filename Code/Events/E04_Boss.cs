...
namespace Celeste.Mod.XaphanHelper.Events
{
    class E04_Boss : CutsceneEntity
    {
        ...
        {
            return XaphanModule.ModSaveData.SavedFlags.Contains("Xaphan/0_Ch4_Boss_Defeated");
        }

        public bool BossDefeatedCM()
        {
            return XaphanModule.ModSaveData.SavedFlags.Contains("Xaphan/0_Ch4_Boss_Defeated_CM");
        }
        ...
        public IEnumerator Cutscene(Level level)
        {
            if (!BossDefeated() || (BossDefeated() && level.Session.GetFlag("boss_Normal_Mode")) || (BossDefeated() && level.Session.GetFlag("boss_Challenge_Mode")))
            {
                ...
                if (!XaphanModule.ModSaveData.WatchedCutscenes.Contains("Xaphan/0_Ch4_BossStart") || HasGolden())
                {
                    yield return null;
                    LeftGate.Move();
                    RightGate.Move();
                ...
                else
                {
                    ...
                    {
                        Scene.Add(new CS04_BossDefeated(player, boss));
                    }
                    else
                    {
