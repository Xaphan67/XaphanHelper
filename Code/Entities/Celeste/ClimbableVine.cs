    ...
    class ClimbableVine : Solid
    {
        ...
        private static void onPlayerUpdate(On.Celeste.Player.orig_Update orig, Player self)
        {
            ...
            foreach (ClimbableVine vine in vines)
            {
                ...
                else if (((self.Right <= vine.Left || self.Left >= vine.Right) && self.Bottom >= vine.Top + 1 && self.Top <= vine.Bottom &&  Input.GrabCheck && self.Holding == null && vine.noCollideDelay <= 0f) && !XaphanModule.PlayerIsControllingRemoteDrone())
                {
