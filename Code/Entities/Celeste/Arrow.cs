    ...
    class Arrow : Solid
    {
        ...
        private static void onPlayerUpdate(On.Celeste.Player.orig_Update orig, Player self)
        {
            ...
            foreach (Arrow arrow in arrows)
            {
                ...
                {
                    if (((self.Right <= arrow.Left || self.Left >= arrow.Right) && self.Bottom >= arrow.Top + 1 && self.Top <= arrow.Bottom) && !arrow.destroyed && arrow.inWall && Input.GrabCheck && self.Holding == null && arrow.noCollideDelay <= 0f)
                    {
