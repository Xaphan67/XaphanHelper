...
namespace Celeste.Mod.XaphanHelper.Entities
{
    ...
    public class Drone : Actor
    {
        ...
        {
            base.Added(scene);
            if (!XaphanModule.ModSaveData.startAsDrone.ContainsKey(SceneAs<Level>().Session.Area.LevelSet))
            {
            ...
            else if (!XaphanModule.ModSettings.UseBagItemSlot.Check && !XaphanModule.ModSaveData.startAsDrone[SceneAs<Level>().Session.Area.LevelSet])
            {
                RemoveSelf();
            }
            else
        ...
        private void ReloadLevel(Level level)
        {
            level.DoScreenWipe(false, delegate
            {
                ...
                foreach (DroneSwitch droneSwitch in level.Tracker.GetEntities<DroneSwitch>())
                {
                    ...
                    if ((!droneSwitch.onlyOnce && !droneSwitch.FlagRegiseredInSaveData() && droneSwitch.startSpawnPoint == level.Session.RespawnPoint) || (droneSwitch.onlyOnce && droneSwitch.resetOnDeath && (string.IsNullOrEmpty(droneSwitch.noResetOnDeathFlag) || (!string.IsNullOrEmpty(droneSwitch.noResetOnDeathFlag) && !level.Session.GetFlag(droneSwitch.noResetOnDeathFlag)))))
                    {
